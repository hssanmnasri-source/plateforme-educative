...
class CertificateService {
    ...
    async checkCourseCompletion(userId, courseId) {
        try {
            ...
            const courseService = (await import('./course.service')).default;

            // Get total counts from course
            const [courseResult, quizzesResult, exercisesResult] = await Promise.all([
                courseService.getCourseById(courseId),
                quizService.getQuizzesByCourse(courseId),
                exerciseService.getExercisesByCourse(courseId)
            ]);

            // Count total lessons from all modules
            let totalLessons = 0;
            if (courseResult.success && courseResult.course.modules) {
                courseResult.course.modules.forEach(module => {
                    if (module.lessons) {
            ...
            const totalQuizzes = quizzesResult.success ? quizzesResult.quizzes.length : 0;
            const totalExercises = exercisesResult.success ? exercisesResult.exercises.length : 0;

            // Check if everything is completed
            const lessonsComplete = totalLessons > 0 ? completedLessons.length >= totalLessons : true;
            const quizzesComplete = totalQuizzes > 0 ? completedQuizzes.length >= totalQuizzes : true;
            const exercisesComplete = totalExercises > 0 ? completedExercises.length >= totalExercises : true;

            const isComplete = lessonsComplete && quizzesComplete && exercisesComplete;

            console.log('📊 Course completion check:', {
                lessonsComplete: `${completedLessons.length}/${totalLessons}`,
                quizzesComplete: `${completedQuizzes.length}/${totalQuizzes}`,
                exercisesComplete: `${completedExercises.length}/${totalExercises}`,
                isComplete
            });
